import type { ManagerSalesInsights, SalesInsightCard } from '@synchem-sfa/shared-types';
...
export function SalesInsightsSection({ enabled }: { enabled: boolean }) {
  const { t, languageHeader } = useI18n();

  const query = useQuery({
    queryKey: ['sales-insights-manager'],
    queryFn: async () => {
      const res = await fetchApi<InsightsResponse>('/api/v1/sales-insights/manager', languageHeader);
      return res.data;
    },
    enabled,
  });
  ...
  return (
    <PageSection title={t('salesInsights.title')}>
      <Spin spinning={query.isLoading}>
        {query.data?.insights.length ? (
          ...
          <Alert type="success" showIcon message={t('salesInsights.empty')} />
        )}
      </Spin>
    </PageSection>
  );
